...
当前 frontend body phase 已经能稳定发布 literal、identifier、call、attribute chain、assignment、subscript 等表达式类型，但 `UnaryExpression`、`BinaryExpression`、`ConditionalExpression` 仍被统一放在显式 deferred 集合里。这导致三个直接后果：
...
当前 `FrontendExpressionSemanticSupport.resolveRemainingExplicitExpressionType(...)` 仍将以下节点直接发布为显式 deferred：

- `BinaryExpression`
- `UnaryExpression`
- `ConditionalExpression`

这说明当前缺口不在 parser，也不在 AST 建模，而在 shared expression semantic support 尚未为 unary/binary 建立正式规则。
