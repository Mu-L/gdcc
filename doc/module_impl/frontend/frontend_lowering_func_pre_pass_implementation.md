...
> 本文档作为 frontend lowering 中 function pre-pass 的长期事实源，记录当前已经稳定落地的 function-shaped lowering scaffold、`FunctionLoweringContext` 合同、preparation pass 行为边界、pre-pass 阶段的 shell-only `LirModule` 约束，以及对后续 CFG/body lowering 仍然有效的架构反思。本文档替代原 `frontend_per_function_lowering_scaffold_plan.md` 的已完成实施内容，不再保留阶段切分、完成记录或验收流水账。
...
- 一旦默认 pipeline 继续进入 `FrontendLoweringBuildCfgPass` / `FrontendLoweringBodyInsnPass`，supported property initializer 会被 materialize 为真实 helper body，而不是长期停留在 shell-only
