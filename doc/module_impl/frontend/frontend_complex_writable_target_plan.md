...
- 已完成：新增 nested dynamic key mutating call 的 body 单测，明确要求“inner runtime-gated call 返回 continuation block 后，outer subscript read/write 必须继续挂到该 block”，并把 base/key 单次求值与 source order 一起固定下来。
...
- 已完成：新增真实引擎 helper family-matrix 回归，覆盖 `Array` / `Dictionary` / `Object` -> false 与 `String` / representative `Vector*` / `Packed*Array` -> true。
...
- 已完成：新增 frontend-to-C 引擎集成测试，覆盖 typed `PackedInt32Array` property-backed mutating receiver 与 key/index 带 side effect 的 subscript route。
- 已澄清：side-effect key route 的引擎集成验收当前应锚定在“object-internal `Variant` key carrier + outer `Dictionary` route”这条语义线上；把 non-null 值直接通过 GDExtension `Variant` 形参传入会落到 `GDEXTENSION_VARIANT_TYPE_NIL` 形参 ABI 限制，而更窄的 typed-dictionary 参数/字段问题会把测试耦合到与 Step 8 无关的崩溃面。
- 已澄清：`String` / `Vector*` 在 Godot 4.5.1 当前并不存在可直接拿来做“mutating method 引擎集成”的 instance non-const builtin method；Step 8 对这些 family 的验收面应收口为 runtime helper family-matrix，而不是伪造不存在的 method coverage。
...
- runtime helper 对 `String` / representative `Vector*` / `Packed*Array` 返回 true

### 引擎集成测试

- `PackedInt32Array.push_back(...)`
- typed `PackedInt32Array` property-backed mutating receiver 的真实运行结果
- `String` / `Vector*` family 收口为 runtime helper matrix 覆盖，而不是伪造 mutating method 集成
