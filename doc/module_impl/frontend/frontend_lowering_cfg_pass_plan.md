...
- negative path：
  ...
  - 若 cfg pass 看到了 compile gate 本应挡住的 unsupported subtree，不得 silent skip
...
这一阶段需要在两个方案中二选一，推荐优先 A：

方案 A：先发布 metadata，再延后 LIR block materialization
...
方案 B：对子集直接 materialize 到 `LirFunctionDef`

- 只对“同一次 landing 内可闭合”的结构写 block
- 写 block 时必须同步设置 `entryBlockId`
- 至少要保证 serializer 不会因 entry 缺失而失败
- 仅当 terminator 也可同步合法生成时，才把该结构放入默认 pipeline

基于当前 LIR 约束，推荐先采用方案 A。
...
- 采用方案 B 时：
  - 每个已 materialize 的 function 都必须有合法 entry block
  - 所有已 materialize block shape 都有稳定测试锚点
  - backend / serializer 不会因半成品函数崩溃
- 两个方案共同的 negative path：
