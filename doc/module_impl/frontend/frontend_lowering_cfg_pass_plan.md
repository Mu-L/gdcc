...
- 已完成。
- `FunctionLoweringContext` 现已从纯 data-record carrier 扩展为带私有 CFG side table 的 lowering-local carrier，但原有 `kind()/sourceOwner()/loweringRoot()/analysisData()` 等访问合同保持不变。
- side table 现固定使用 `FrontendAstSideTable` 保存 AST identity keyed 的 `CfgNodeBlocks` bundle，并发布最小协议：
  - `publishCfgNodeBlocks(...)`
  - `cfgNodeBlocksOrNull(...)`
  - `requireCfgNodeBlocks(...)`
  - `hasCfgNodeBlocks(...)`
- block bundle 类型目前内聚在 `FunctionLoweringContext` 内部，按第一轮 CFG 角色固定为：
  - `BlockCfgNodeBlocks`
  - `IfCfgNodeBlocks`
  - `ElifCfgNodeBlocks`
  - `WhileCfgNodeBlocks`
- duplicate publish 当前会直接 fail-fast；对未发布节点的 `require` 也会 fail-fast，而 query-or-null 继续返回 `null`。
- `FrontendLoweringCfgPassTest` 已补充以下锚点：
  - AST identity keyed 查询不会被同构但非同一 AST 节点误命中
  - nested `if` 节点 bundle 不会互相覆盖
  - duplicate publish fail-fast
  - 未发布的无关节点查询返回 `null`
...
- negative path：
  - compile gate 已封口的 blocked subtree 继续不应进入 walker；若仍出现未处理 statement kind，应视为前置协议破坏并 fail-fast
...
- 已完成。
- `FrontendLoweringCfgPass` 现已从单纯 contract-freeze pass 扩展为 metadata-only CFG skeleton walker：
  - 只消费 `EXECUTABLE_BODY`
  - 继续要求 target function 保持 shell-only
  - 只向 `FunctionLoweringContext` 发布 per-node block bundle
  - 仍不向 `LirFunctionDef` 写入 basic block 或 `entryBlockId`
- 当前 walker 已稳定处理：
  - executable root `Block`
  - `PassStatement`
  - `ReturnStatement`
  - `ExpressionStatement`
  ...
  - `ElifClause`
  - `WhileStatement`
- block naming 当前固定为 deterministic lexical-counter 策略：
  - root entry：`entry`
  - `if`：`if_then_<n>` / `if_false_<n>` / `if_merge_<n>`
  - `elif`：`elif_body_<n>` / `elif_false_<n>`
  - `while`：`while_cond_<n>` / `while_body_<n>` / `while_exit_<n>`
- 对没有显式 `else` / `elif` false-entry 的 `if`，`elseOrNextClauseEntry` 现在允许直接 alias 到 `merge`；对链尾 `elif` 且无显式后继的情况，`nextClauseOrMerge` 也允许直接 alias 到 owning `if` 的 merge block。
- walker 当前会在 lexical block 中维护“当前可继续发布 skeleton 的 block”概念：
  - `return` 会终止当前 lexical path
  - `if/elif/else` 只有存在 fallthrough path 时才把后续 statement 继续挂到 merge
  - fully-terminating `if/else` 之后的后续 source statement 不会再被错误分配 CFG bundle
- compile gate 继续负责阻止 `assert` / `for` / `match` / `lambda` / `ConditionalExpression` 等 blocked subtree 进入 lowering；cfg walker 不再重复扫描这批类别。若仍然遇到未处理 statement kind，当前实现把它当作 lowering 前置协议被破坏并 fail-fast。
- `FrontendLoweringCfgPassTest` 当前新增锚点覆盖：
  - nested `if/elif/else/while` 的 deterministic block bundle 发布
  - metadata-only 行为下 target function 继续保持 shell-only
  - property initializer context 继续不参与 CFG materialization
  - fully-terminating `if/else` 后的 lexical remainder 不会继续发布 skeleton

### 3.5 第五步：接入默认 pipeline
...
- negative path：不得出现“部分函数写了 block、但没有 entry / 没有后续可消费路径”的中间态悄悄流出 public lowering 入口

当前状态（2026-03-28）：

- 已完成。
- 默认 `FrontendLoweringPassManager` 现在固定接入：
  1. `FrontendLoweringAnalysisPass`
  2. `FrontendLoweringClassSkeletonPass`
  3. `FrontendLoweringFunctionPreparationPass`
  4. `FrontendLoweringCfgPass`
- `FrontendLoweringCfgPass` 在默认 pipeline 中仍保持 metadata-only 合同：
  - 只向 `FunctionLoweringContext` 发布 per-node CFG side table
  - 不向 `LirFunctionDef.basicBlocks` 写入 block
  - 不设置 `entryBlockId`
  - 不改变默认 public lowering 入口返回的 shell-only `LirModule`
- `FrontendLoweringPassManager` 新增 package-local `lowerToContext(...)` 作为测试/内部验证钩子，用于在不扩张 public API 的前提下检查默认 pipeline 发布的 lowering context；公开方法仍然只有 `lower(...)`。
- `FrontendLoweringPassManagerTest` 当前新增锚点覆盖：
  - 默认 pipeline 会为 compile-ready executable body 发布 CFG side-table metadata
  - property initializer context 继续不参与 CFG metadata materialization
  - 默认 lowering 返回的 `LirModule` 继续可以被 serializer 视为 shell-only 中间态
  - compile-blocked module 会在 analysis 后停止，`lirModule` 与 `functionLoweringContexts` 都不会发布，因此 cfg pass 不会被默认 pipeline 误执行
