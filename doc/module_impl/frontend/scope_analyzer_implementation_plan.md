...
> 本文档作为 `frontend.sema.analyzer` 中 scope analyzer 的专项实施计划，定义 scope analyzer 的职责边界、AST -> Scope side-table 建图规则、与 Godot 的对齐结论、分阶段实施步骤，以及每个阶段的验收细则。
...
1. 在 `FrontendSemanticAnalyzer` 中接入 scope analyzer，使 analyze 结果不再只有 annotations 和 skeleton，而是额外拥有 AST -> Scope 的 side-table。
2. scope analyzer 只负责建立 lexical/container scope graph，以及填充最小必要的初始绑定事实。
3. 普通局部变量、capture、pattern binding、更多 type-meta、成员绑定结果等事实，继续由后续 binder/body phase 渐进填充。
...
- 要做：
  ...
  - 预填最小必要绑定事实，例如 callable parameters
- 不做：
  - 普通局部变量的完整绑定
  - capture 推导
...
- 向 `analysisData.scopesByAst()` 写入 AST -> Scope 映射
...
- 建 scope 对象
- 连接 parent 链
- 为 AST 对象记录“当前应使用的 lexical scope”
- 预填 callable parameters
...
> 某个 AST 对象在后续语义分析中应使用的当前 lexical scope。

这意味着 side-table 的 key 不仅应覆盖 scope owner，还应覆盖后续 binder 需要在其上发起名称解析的 AST 对象。

### 5.2 必须覆盖的 AST 对象类别

初版至少应为以下 AST 对象挂 scope：
...
| 语义来源 | 实际携带 scope 的 AST 对象 | 备注 | 建议枚举值 |
| --- | --- | --- | --- |
...
- 初版 scope analyzer 仍然要给 inner class 建立独立 lexical boundary
- 初版允许使用 placeholder / deferred metadata 策略，只要 lexical scope graph 是稳定的
- 不能因为 metadata 未完备而放弃 inner class subtree 的 scope side-table

推荐的阶段性落地顺序是：

1. 先保证 inner class subtree 具有独立 `ClassScope`
2. outer class 只通过已冻结的 type-meta 链影响其解析
3. 更完整的 inner-class skeleton/interface 信息由后续 phase 补齐
...
- 每个 `MatchSection` 必须拥有独立 branch `BlockScope`
- `PatternBindingExpression` 至少要在 side-table 中拿到正确 scope
- 是否立即把 pattern binding 注册成 local，由后续 binder/body phase 决定

换言之，初版至少要先把结构边界建对，不能把 match 所有分支错误地压成同一个 scope。
...
### 7.6 Phase 5：补最小 binding 预填

### 目标

在不提前侵入 binder 责任的前提下，为后续 phase 补充最小必要绑定事实。

### 实施内容

- 把 callable parameters 定义到对应 `CallableScope`
- 明确 `for` iterator 继续 deferred 到后续 pass
- 其余普通 locals、captures、pattern bindings 继续 deferred
...
- `CallableScope` 至少已包含 parameters
- parameter lookup 形状符合现有 `CallableScope` 协议
...
| 顶层/类/callable scope 建图 | - | Y | Y | Y | Y | Y |
...
7. `CallableScope` 至少已包含 parameters。
...
- 如果在 scope analyzer 阶段过早注册普通 locals、captures、pattern bindings，就会把声明顺序、初始化器可见性、捕获规则等复杂语义硬塞进 scope phase。

控制策略：

- 初版严格限制为结构性 scope graph + 最小 binding 预填
- 对普通 locals/captures/pattern bindings 统一 deferred
...
- 明确 `scopesByAst` 的语义是“该 AST 对象的当前 lexical scope”
...
6. 最后补 callable parameters 的最小预填。
7. `for` iterator 预填在 scope analyzer 首轮之外，留给后续独立 pass。
8. 所有结构稳定后，再规划 binder/body phase 如何消费这些 scope。
...
- parameters 是否已经预填到 `CallableScope`
