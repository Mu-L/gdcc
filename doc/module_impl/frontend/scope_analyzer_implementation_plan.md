...
- 状态：Phase 0 / Phase 1 / Phase 2 / Phase 3 / Phase 4 已完成（scope phase 骨架、总控接线、callable 主干、控制流 block scope graph、inner class lexical boundary 与 immediate-inner type-meta 发布已落地，Phase 5+ 待继续实现）
- 更新时间：2026-03-13
- 适用范围：
...
当前它已经拥有独立的 scope phase worker，并已完成顶层脚本、callable、控制流 block 以及 inner class 的 AST scope 建图。
...
完整 scope analyzer 最终仍会继续扩展到更多 binder/body phase 需要消费的节点。当前实现已经覆盖顶层脚本 /
callable / 控制流 block / inner class lexical boundary 的主干子集；变量绑定与更多 type-meta
消费点继续按后续 phase 推进：
...
建议按下表建立结构性映射。当前实现已经物化：

- `SourceFile` -> 顶层脚本 `ClassScope`
- `ClassDeclaration` -> inner class `ClassScope`
- `FunctionDeclaration` / `ConstructorDeclaration` / `LambdaExpression` -> `CallableScope`
- callable `body: Block` -> 对应 `BlockScope`
- 普通 `Block`、控制流 body、`MatchSection` body -> 对应的 `BlockScope`

仍保持 deferred 的是 variables/captures/pattern bindings 等“向 scope 注入绑定”的后续动作，
而不是这些 lexical boundary 本身：
...
inner class 是首轮 scope graph 中最容易被误跳过的结构性边界之一。当前模块骨架已经通过
`FrontendSourceClassRelation` 显式记录：

- 每个 `FrontendSourceUnit` 的 `topLevelClassDef`
- 每个 inner `ClassDeclaration` 对应的 `FrontendInnerClassRelation`，其中已冻结 `lexicalOwner`、`sourceName`、`canonicalName` 与 canonical `LirClassDef`

这些 inner class metadata 现已同时接入 canonical `ClassRegistry` 注册与 scope analyzer 的 immediate-inner type-meta
发布，因此它们已经不再只是“扁平 source-local pair”。Phase 4 已不再因此推迟 lexical scope graph；后续
binder/type 解析阶段应继续消费这些已冻结的 identity facts，而不是重新靠名字或顺序猜测。本计划当前冻结如下策略：

- `FrontendScopeAnalyzer` 在进入 `ClassDeclaration` 时，必须通过对应 `FrontendSourceClassRelation` 中的显式 relation 物化独立 `ClassScope`
- `SourceFile` 顶层 `ClassScope` 与每个 inner `ClassScope` 都只发布其 direct inner classes 的 `type-meta`
- `ClassDeclaration.body` 作为类成员容器复用 owning `ClassScope`，不额外生成 `BlockScope`
- nested callable / block / expression subtree 继续在该 inner-class boundary 下建图
- 若某个 inner class subtree 因 skeleton phase 已被拒绝而没有已发布 relation，scope analyzer 只跳过该 subtree，不打断同一 source 中其他合法节点
- source-local inner class 已通过 canonical name 注入 `ClassRegistry`；outer class 仍只通过已冻结的 lexical parent / type-meta 协议影响其解析，不得泄漏 outer value/function
...
进一步明确 Phase 3 的 branch 规则：

- `MatchSection` subtree 在进入 section 时就切换到其独立 branch `BlockScope`
- `PatternBindingExpression`、其它 pattern 表达式、`guard` 与 section `body` 共享这一 branch scope
- 这样后续 binder/body phase 即使继续延后“真正把 pattern binding 注册成 local”的动作，也已经有稳定的 branch scope 可供 guard/body 共用
...
- [x] `FrontendModuleSkeleton` 已改为以 `FrontendSourceClassRelation` 显式记录 source ownership：每个 `FrontendSourceUnit` 绑定统一的顶层 `name` / `topLevelClassDef`，并依赖“顶层 gdcc 类 `sourceName == canonicalName`”这一冻结不变量；同一 relation 下还保存零到多个带 `lexicalOwner`、`sourceName`、`canonicalName` 的 inner class relation；`innerClassDefs()` / `classDefs()` 保留兼容视图，`allClassDefs()` 提供完整扁平集合。
- [x] `FrontendScopeAnalyzer` 已迁移到基于 `gdparser ASTWalker` 的实现，并先基于 `moduleSkeleton.sourceClassRelations()` 中显式的 `SourceUnit -> topLevelClassDef` 关系为每个 `SourceFile` 物化顶层脚本 `ClassScope`；Phase 4 再在同一 relation 上继续消费 inner class owner pair，为 nested `ClassDeclaration` 物化 `ClassScope`，不再依赖任何索引或名字猜测。
...
- [x] 已通过正反测试明确 Phase 2 当时的边界：控制流 body block、`MatchSection` branch scope、inner class lexical boundary 与 parameter prefill 仍保持 deferred，没有在本阶段被伪装成“已支持”；其中 inner class lexical boundary 已在 2026-03-12 的 Phase 4 实现中落地，parameter prefill 仍 deferred。
...
- `match guard` 与 pattern 表达式必须落在所属 `MatchSection` 的 branch scope 下，不能错误回退到 enclosing 外层 scope
...
- 条件表达式与 loop iterable 在 side-table 中可回溯到 enclosing 外层正确 scope
- `PatternBindingExpression`、其它 match pattern 表达式与 `guard` 在 side-table 中可回溯到所属 `MatchSection` 的 branch scope

### 当前状态（2026-03-11）

- [x] `FrontendScopeAnalyzer` 已为 `IfStatement`、`ElifClause`、`else` body、`WhileStatement` body、`ForStatement` body 建立独立 `BlockScope`，并把 `IF_BODY`、`ELIF_BODY`、`ELSE_BODY`、`WHILE_BODY`、`FOR_BODY` 等 `BlockScopeKind` 写入稳定事实。
- [x] `FrontendScopeAnalyzer` 现在也会为 walker 遍历到的普通独立 `Block` 生成 `BLOCK_STATEMENT` scope，避免把独立 lexical block 与 callable/control-flow body 混用成同一个来源。
- [x] `if` / `elif` / `while` 的条件表达式，以及 `for` 的 `iteratorType` / `iterable`，都继续在进入 body 之前的 enclosing 外层 scope 下挂载 side-table。
- [x] `match` 语义已按 section branch 收口：`MatchStatement.value` 仍在外层 scope，下钻到每个 `MatchSection` 后会切换到独立 branch `BlockScope`，其 pattern、`PatternBindingExpression`、`guard` 与 body block 共享该 scope，且不同 section 彼此隔离。
- [x] Phase 3 交付时仍未扩大到 Phase 4 / Phase 5 的边界：当时 inner class lexical boundary 继续 deferred，`for` iterator、pattern binding、普通 locals 与 parameters/captures 也仍未在建图阶段被预填成可解析绑定；其中 inner class deferred 已在 2026-03-12 的 Phase 4 任务中收回，其余 deferred 项保持不变。
- [x] 已补齐 `FrontendScopeAnalyzerTest` 的正反测试，并完成 targeted tests 回归：`GdScriptParserServiceDiagnosticManagerTest`、`FrontendParseSmokeTest`、`FrontendScopeAnalyzerTest`、`FrontendSemanticAnalyzerFrameworkTest` 已通过。
...
- [x] `FrontendSourceClassRelation` 已扩展为显式保存 source-local inner `ClassDeclaration -> LirClassDef` relation；`innerClassDefs()` 仅保留兼容视图，避免 analyzer 再靠顺序或名字猜测 skeleton 归属。
...
- 当前 skeleton 最小 type-scope 与真实 scope analyzer 都已经消费 source-local inner class metadata。风险已从“metadata 尚未接入”转移为“两个发布路径若不共享规则，就可能在 immediate-inner type-meta 可见性上再次漂移”。

控制策略：

- skeleton 与 analyzer 复用同一份 immediate-inner type-meta 发布规则
- 用 parity tests 锚定 shared `ScopeTypeResolver`、skeleton 与 analyzer 的行为一致性
