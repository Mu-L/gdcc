...
- 状态：Phase 0 / Phase 1 / Phase 2 / Phase 3 / Phase 4 已完成（scope phase 骨架、总控接线、callable 主干、控制流 block scope graph 与 inner class lexical boundary 已落地，Phase 5+ 待继续实现）
- 更新时间：2026-03-13
- 适用范围：
...
这些 inner class metadata 当前仍未接入全局 `ClassRegistry`，但它们已经不再只是“扁平 source-local pair”。Phase 4 已不再因此推迟 lexical
scope graph；后续 inner-class registry publish phase 只应继续消费这些已冻结的 identity facts，而不是重新靠名字或顺序猜测。本计划当前冻结如下策略：
...
- inner class 当前仍暂不注入 `ClassRegistry`；这只是发布时序未完成，而不是 identity 模型缺失；outer class 仍只通过已冻结的 lexical parent / type-meta 协议影响其解析
...
- 当前 skeleton 已能收口 source-local inner class metadata，并已接入 Phase 4 lexical-boundary 流程；但这些 metadata 还未完全接入全局 `ClassRegistry`，仍可能导致实现者误以为必须等待“完整全局 metadata”后再做后续 type-meta/binder 工作。
