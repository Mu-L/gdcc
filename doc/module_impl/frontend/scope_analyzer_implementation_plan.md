...
- 每个 inner `ClassDeclaration` 对应的 source-local `LirClassDef`（通过 `FrontendInnerClassRelation`）

这些 inner class metadata 仍未接入全局 `ClassRegistry`，但 Phase 4 已不再因此推迟 lexical
scope graph。本计划当前冻结如下策略：
...
- inner class 仍暂不注入 `ClassRegistry`；outer class 只通过已冻结的 lexical parent / type-meta 协议影响其解析
...
- [x] `FrontendModuleSkeleton` 已改为以 `FrontendSourceClassRelation` 显式记录 source ownership：每个 `FrontendSourceUnit` 绑定一个 `topLevelClassDef`，以及零到多个 source-local `ClassDeclaration -> LirClassDef` inner class relation；`innerClassDefs()` / `classDefs()` 保留兼容视图，`allClassDefs()` 提供完整扁平集合。
