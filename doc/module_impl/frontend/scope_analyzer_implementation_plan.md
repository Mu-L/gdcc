...
- 状态：Phase 0 / Phase 1 / Phase 2 / Phase 3 已完成（scope phase 骨架、总控接线、callable 主干与控制流 block scope graph 已落地，Phase 4+ 待继续实现）
- 更新时间：2026-03-11
- 适用范围：
...
inner class 是第一阶段最需要明确的难点之一。当前模块骨架已经通过 `FrontendSourceClassRelation` 显式记录 `FrontendSourceUnit` 对其顶层类和同源 inner class skeleton 的归属关系，但这些 inner class metadata 仍停留在 source-local skeleton 层，还没有接入 `ClassRegistry` 或被 `FrontendScopeAnalyzer` 作为 lexical boundary 消费。
...
- [x] `FrontendModuleSkeleton` 已改为以 `FrontendSourceClassRelation` 显式记录 source ownership：每个 `FrontendSourceUnit` 绑定一个 `topLevelClassDef` 和零到多个同源 inner class skeleton；`classDefs()` 仅保留顶层兼容视图，`allClassDefs()` 提供完整扁平集合。
- [x] `FrontendScopeAnalyzer` 已迁移到基于 `gdparser ASTWalker` 的实现，并基于 `moduleSkeleton.sourceClassRelations()` 中显式的 `SourceUnit -> topLevelClassDef` 关系，为每个 `SourceFile` 物化顶层脚本 `ClassScope`，不再依赖 `units()` 与 `classDefs()` 的索引配对。
...
- [x] Phase 3 仍未扩大到 Phase 4 / Phase 5 的边界：inner class lexical boundary 继续 deferred，`for` iterator、pattern binding、普通 locals 与 parameters/captures 也仍未在建图阶段被预填成可解析绑定。
...
- 当前 skeleton 已能收口 source-local inner class metadata，但这些 metadata 还未完全接入全局 `ClassRegistry` / Phase 4 lexical-boundary 流程，可能导致实现者误以为必须等待“完整全局 metadata”后再做 scope graph。
