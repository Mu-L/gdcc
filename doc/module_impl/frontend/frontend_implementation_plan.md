...
- 关联事实源：
  ...
  - 作用域与共享 resolver 规划：
    - `doc/module_impl/scope_architecture_refactor_plan.md`
...
- `FrontendModule`
  ...
  - `ClassRegistry classRegistry`（engine/builtin/utility metadata + 后续注入 gdcc classes，同时作为 global scope root）
- `AnalysisResult`
  ...
  - `IdentityHashMap<Node, Scope>` 或等价 `scopeByNode` side-table（记录 AST 节点对应的当前作用域）
...
4. **Resolve Body**：处理 “表达式级信息”
   - 基于 `Scope` 接口构建函数体作用域树（`ClassRegistry` 作为 global scope root，按节点创建 `ClassScope` / `CallableScope` / `BlockScope`）
   - 类型推断（含控制流合流）
   - 调用点决议（含 overload 选择、默认参数补全规则，method/property owner lookup 应优先复用 `scope` 包中的共享 resolver）
...
实现上建议不要让 binder 在各处手写 `if/else + ClassRegistry` 分支，而是：

- 以 `ClassRegistry` 作为 global scope root；
- 在类 / 函数 / block 节点上创建新的 `Scope` 并设置 parent；
- identifier 的无 base 解析按语法上下文分别走 `Scope.resolveValue(...)` / `Scope.resolveFunctions(...)` / `Scope.resolveTypeMeta(...)`；
- `TypeRef`、`CastExpression`、`TypeTestExpression` 等 type-position 解析禁止退回宽松的 `findType(...)`；
- receiver/member 解析则按 base 种类分流：value receiver 复用共享 method/property resolver，`TYPE_META` receiver 走 static access / constructor 路径。
...
4. **先区分 value receiver 与 `TYPE_META` receiver**：前者进入 instance method 路径，后者进入 `call_static_method` / `construct_*` 等静态语义路径
...
后续实现建议优先把“候选收集 + ownerDistance + 排序规则”抽到 `dev.superice.gdcc.scope` 中的共享 method resolver，backend / frontend 仅分别消费结果，而不要长期维护两套并行 resolver。

### 3.4 属性与索引语义

对照后端实现（`PropertyAccessResolver`、`IndexLoadInsnGen/IndexStoreInsnGen`），前端建议在语义层区分三类访问。

其中 owner/property metadata lookup 建议后续下沉到 `dev.superice.gdcc.scope` 的共享 property resolver；backend 专用的 receiver 渲染、C 表示转换与最终发射逻辑仍保留在 `backend.c`：
...
- expressions：
  ...
  - `AttributeExpression`：
    ...
    - 若 base 绑定为 `TYPE_META`：
      - `.CONST` / `.ENUM_VALUE` -> `load_static`
      - `.static_method(...)` -> `call_static_method`
      - `.new(...)` -> `construct_builtin` / `construct_object`
...
1. 引入 `Scope` / `ScopeValue` / `ScopeOwnerKind` 等基础协议，并让 `ClassRegistry` 实现 global scope root。
2. `TypeRef` -> `GdType` 解析：
   - 复用 `ClassRegistry.tryParseTextType(...)` / `ClassRegistry.findType(...)`
3. 生成 `_default_*` / `_field_init_*` 的隐藏函数声明（body 在 M3/M4 完成）
4. 将 `defaultValueFunc` / `initFunc` 写入 `LirParameterDef` / `LirPropertyDef`
...
1. 实现 `frontend.sema`：
   - 基于 `Scope` 链的绑定规则（局部/参数/self/property/utility）
   - 为 class / callable / block 建立显式 scope 实现与 `scopeByNode` side-table
   - 局部类型推断：沿语句顺序推断与校验
...
1. 引入 `frontend.sema.call`，并优先复用 `dev.superice.gdcc.scope` 中的共享 resolver：
   - global utility -> `FunctionSignature`
   - builtin/engine/gdcc method -> candidates + selection
   - property / method owner lookup 不再长期依赖 backend 包内私有逻辑
