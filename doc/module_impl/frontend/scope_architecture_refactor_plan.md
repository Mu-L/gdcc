...
- 状态：事实源维护中（`Scope` / frontend scope chain / `ScopeTypeResolver` / `ScopeSignalResolver` / `FrontendScopeAnalyzer` 已落地；frontend variable/binder/body phase 接入待实施）
- 更新时间：2026-03-15
- 适用范围：
...
- `signal` 的无 receiver 名称解析与 receiver-based metadata lookup 已纳入 `ClassScope` / `ScopeSignalResolver`，但 `await`、`.emit(...)`、`self.some_signal` 的完整 binder 语义仍未闭环
- `self` 与 signal 的完整用户可见 diagnostics 仍由 frontend binder 的后续阶段负责
...
当前 frontend 已经稳定落地 `skeleton -> scope analyzer` 主链路。后续 variable/binder/body phase 必须建立在这条已发布的 scope graph 与 shared resolver 事实上，不能重新发明第二套 lookup 语义。
...
当前还需继续保持 deferred 的 scope-prefill / binding 内容包括：
...
这些内容应由后续独立 variable/binder phase 接手，而不是回写进 `FrontendScopeAnalyzer`。
...
### 6.4 `self` 尚未纳入当前 `Scope` 协议，signal 仍未完成 binder 闭环
...
本次最小可行修复状态（2026-03-15）：

- [x] 建立本次修复的任务清单与状态同步入口。
- [x] 让 `ClassScope` 在 missing super metadata 时 fail fast，而不是继续把它当成普通 miss。
- [x] 为 `ClassScope` 的 value/function/signal 路径补齐正反两方面测试，并运行 targeted tests。

本次用于冻结行为的 targeted tests：
...
### 6.7 scope graph 已落地，但 variable prefill 仍是独立前置任务

当前 `FrontendScopeAnalyzer` 只发布 lexical/container graph，不负责往 `CallableScope` / `BlockScope` 写入运行时前端绑定。

参数与普通局部变量这条 MVP 落地路径，详见：
...
后续 `FrontendVariableAnalyzer` 或 binder phase 在接线前至少需要冻结：

- 同一 callable 内的 parameter/local/capture shadowing 规则
- 参数默认值的可见性顺序
- 局部变量 initializer 的可见性顺序
