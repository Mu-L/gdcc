...
- 状态：事实源维护中（executable-body CFG build / body lowering、constructor materialization、compound assignment、`StopNode.kind` 空-return 图修复均已落地；property initializer 与 parameter default 仍保持 shell-only / 未接通）
- 更新时间：2026-04-05
- 适用范围：
...
- public lowering 返回值是“executable body 已 materialize、property-init shell 仍保留”的 `LirModule`
...
- `PROPERTY_INIT`
  - 只保留 function shell，不进入 CFG build / body lowering
