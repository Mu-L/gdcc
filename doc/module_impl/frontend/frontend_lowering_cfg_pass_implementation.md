...
- 状态：事实源维护中（executable-body CFG build / body lowering、property-initializer CFG publication、constructor materialization、compound assignment、`StopNode.kind` 空-return 图修复均已落地；property initializer body lowering 与 parameter default 仍未接通）
- 更新时间：2026-04-06
- 适用范围：
...
- 明确非目标：
  ...
  - 不在这里把 property initializer / parameter default 接到 executable-body body pass
...
- public lowering 返回值是“executable body 已 materialize、property-init helper 已拥有 frontend CFG 但 LIR body 仍保留 shell-only”的 `LirModule`
...
- `PROPERTY_INIT`
  ...
  - 暂不 materialize `LirBasicBlock` / instruction
...
- `FrontendLoweringBodyInsnPass` 本体只保留 executable-body context 调度，以及 property-init / parameter-default 的 shell-only guard rail
...
- `FrontendLoweringPassManagerTest`
  ...
  - property initializer CFG publication + shell-only LIR boundary
