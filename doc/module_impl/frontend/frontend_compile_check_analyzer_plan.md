...
- 现有 `FrontendSemanticAnalyzerFrameworkTest` 对默认 `analyze(...)` 的断言不需要因 compile-only 新 phase 而改写
- compile-only 入口新增后，默认语义主链的 diagnostics snapshot 行为保持不变
- compile-only 入口运行结束后，`analysisData.diagnostics()` 等于最新 `diagnosticManager.snapshot()`
...
- `assert` 出现在 supported executable body 中时，compile-only 路径会新增 `sema.compile_check` error
- `assert` 在默认 `analyze(...)` 路径中不新增 compile-only error
- 上述 6 类表达式只要出现在 compile surface 中，compile-only 路径都会被拦截
- 这些节点当前的 source-level typed contract 不发生反向收紧
...
- deferred warning 在 compile mode 下会被补成真正的 compile blocker
- blocked/failed/unsupported 即使未来某条上游 owner 规则回归，也不会静默漏过 compile gate
- compile check 不会因为同一 root 上有多个非稳定条目而制造大量重复 error
...
- compile-only 分析入口可以作为未来 lowering 的唯一合法前置入口
- 文档中明确写出“共享 `analyze(...)` 结果不能直接视为 lowering-ready”
- compile gate 的存在不会改变共享语义事实，只改变“是否允许继续编译”
...
- [ ] `FrontendSemanticAnalyzer.analyze(...)` 仍保持共享语义入口，不附带 compile-only phase
- [ ] compile-only 分析入口在 type check 之后追加 `FrontendCompileCheckAnalyzer`
- [ ] `FrontendCompileCheckAnalyzer` 为 diagnostics-only，不新增也不改写 side table
- [ ] `sema.compile_check` 被正式引入并固定为 `error`
- [ ] `AssertStatement` 在 compile mode 中被显式拦截
- [ ] `ArrayExpression`、`DictionaryExpression`、`PreloadExpression`、`GetNodeExpression`、`CastExpression`、`TypeTestExpression` 在 compile mode 中被显式拦截
- [ ] 所有已发布的 `BLOCKED` / `DEFERRED` / `FAILED` / `UNSUPPORTED` 在 compile mode 中都不能静默漏过
- [ ] `DYNAMIC` 不被 compile check 误判为 blocker
- [ ] compile check 不会对已有 upstream error 的同一 anchor 重复补 generic error
- [ ] diagnostics snapshot 在 compile-only 入口结束后保持一致
- [ ] 文档明确写出 `assert` 与 6 类表达式只是 temporary compile intercept，不代表 source contract 已改变
- [ ] 文档明确写出该 analyzer 未来不进入 LSP 路径
