...
若后续 frontend 需要统一 superclass 的用户可见展示名，也应从 canonical layer 派生，而不是再持久化额外的 `runtimeName` 字段。
...
- frontend source-facing superclass 合同不暴露 canonical `__sub__` spelling
...
- backend 在非 `zig cc` 工具链下对 `__sub__` 的兼容策略
- 统一 superclass display 文案若要进入更多 frontend 路径，应从 canonical name 派生，而不是新增持久化名字层
...
- mapped top-level canonical superclass 的 `checkAssignable(...)` / `getRefCountedStatus(...)` 必须有正反断言
- DOM/LIR parser/serializer 与 backend codegen 必须继续只消费 canonical `name/super`
