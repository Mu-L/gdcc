...
- H2：assignment 语义与 statement policy 收口
  ...
  - 当前实施拆分（2026-03-18）：
    ...
    - [x] H2.4 已完成：已新增/扩展 `FrontendAssignmentSemanticSupportTest`、`FrontendExprTypeAnalyzerTest`、`FrontendChainBindingAnalyzerTest`；覆盖 bare identifier / attribute property / plain subscript / attribute-subscript success、signal / `TYPE_META` / bare callable / readonly property / assignability failure、compound assignment unsupported、exact `Variant` slot 与 `DYNAMIC` target assignment positive path，以及 value-required nested assignment fail-closed。
    ...
    - [x] H2.6 已完成（2026-03-19）：`FrontendAssignmentSemanticSupport` 已新增 frontend-owned assignment compatibility gate；其公开 API `checkAssignmentCompatible(...)` 只负责 concrete slot 的兼容判断，exact `Variant` slot 不再被 generic `ClassRegistry.checkAssignable(...)` 误判失败。`DYNAMIC` target 的 runtime-open 处理继续保持在 assignment semantic helper 内部，其余 concrete assignment slot 仍回退全局 generic check。当前阶段必须继续明确冻结：MVP 不因该 gate 扩张到 numeric promotion、`StringName` / `String` 互转、`null -> Object` 等更宽隐式转换。
...
- H2 完成后：
  ...
  - exact `Variant` slot 与 `DYNAMIC` target assignment 不再被 generic assignability 误判失败；除此之外的更宽 Godot-style implicit conversion 仍不属于 MVP
- H3 完成后：
...
- [x] H2 已完成（2026-03-19）：已新增 `FrontendAssignmentSemanticSupport` 作为独立 assignment semantic 真源，并从 `FrontendExpressionSemanticSupport` 中移除 assignment；left-hand side 不再走 ordinary expression resolver，而是通过 dedicated assignment-target model 解析 bare identifier / attribute property / plain subscript / attribute-subscript。assignment-success root 现稳定发布 `RESOLVED(void)`，statement-position 不再误发 discarded warning；value-required nested assignment 会 fail-closed；illegal lvalue / assignability failure / compound assignment boundary 现具备明确 `FAILED` / `UNSUPPORTED` contract。`ScopeValue` 现额外承载 bare identifier direct-write 所需的最小 `writable` contract，且 bare identifier property 与 attribute-property 都统一复用 `PropertyDefAccessSupport` 解释 readonly metadata，避免对同一 property 分叉出不同 assignment 结论。assignment compatibility 现通过公开 API `FrontendAssignmentSemanticSupport.checkAssignmentCompatible(...)` 复用 concrete slot 判断，并在 helper 内部额外收口 `DYNAMIC` target 的 runtime-open 语义；MVP 仍不扩张到 numeric promotion、`StringName` / `String` 互转、`null -> Object` 等更宽隐式转换。当前阶段对 `attribute-subscript` 仍按 property value 上的 element mutation 建模，getter-only property 是否额外阻断该 aliasing route 暂留后续里程碑统一收口。已新增/扩展 `FrontendAssignmentSemanticSupportTest`、`FrontendExprTypeAnalyzerTest`、`FrontendChainBindingAnalyzerTest`、`ClassScopeResolutionTest`、`ClassScopeSignalResolutionTest`、`ClassRegistryScopeTest` 与 `ScopeProtocolTest`；通过 assignment / expr / chain / scope 定向测试共同锚定 direct-write contract、公开 compatibility API、`Variant` / dynamic assignment compatibility，以及 value-required fail-closed 边界。
- [x] H3 已完成（2026-03-19）：`FrontendExpressionSemanticSupport` 现新增 `resolveRemainingExplicitExpressionType(...)` 作为 shared 剩余节点真源，`FrontendExprTypeAnalyzer` / `FrontendChainBindingAnalyzer` 不再把 unknown remaining expressions 丢回 generic milestone-G 黑箱；当前显式 deferred set 已冻结为 `BinaryExpression`、`UnaryExpression`、`ConditionalExpression`、`ArrayExpression`、`DictionaryExpression`、`AwaitExpression`、`PreloadExpression`、`GetNodeExpression`、`CastExpression`、`TypeTestExpression` 与 `PatternBindingExpression`，每类节点都拥有单独命名的 deferred reason；`UnknownExpression` 则显式收口为 parser-recovery `UNSUPPORTED`，不再伪装成“合法但暂缓”的 deferred route。已扩展 `FrontendExpressionSemanticSupportTest`、`FrontendExprTypeAnalyzerTest` 与 `FrontendChainBindingAnalyzerTest`，锚定 exhaustive remaining-route contract、dependency provenance 传播，以及 expr/chain diagnostics 不再出现旧的 generic fallback 文案。
- [x] H4 已完成（2026-03-19）：已补齐 `.bind(...)`（`helper.bind(...)`、`self.helper.bind(...)`、`Worker.build.bind(...)`）、callable chain head（`self.helper.call()`、`Worker.build.call()`、blocked/failed callable receiver）、blocked bare call chain-path、callable direct invocation unsupported 变体、subscript+callable integration（`items[0].bind(...)`、`dict["cb"].call()`、`consume(items[0])`）以及 helper-level `ambiguous bare call` / `empty overload set` 覆盖。实现上同步修正 shared callable/method overload matching 的 vararg tail 合同：Godot vararg 调用现按 Variant-packed open tail 处理，不再错误要求对每个尾参数证明严格 `T -> Variant` assignability，从而使 `Callable.bind(...)` 等合法脚本稳定通过 exact route；旧有 “value-required nested assignment in chain argument” 测试也已按当前 documented fail-closed `UNSUPPORTED` contract 对齐，而不再陈旧地期待 `DEFERRED`。
