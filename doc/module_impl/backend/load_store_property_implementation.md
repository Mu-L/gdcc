...
- 对 value-semantic backing field：
  ...
  - setter-self 直写必须直接把 copy ctor 结果写回 backing field，不得走“copy temp -> field = temp -> destroy temp”
...
- 非对象 destroyable/value-semantic 槽位写入也有对应硬约束：
  - 对现有 `BORROWED` source，必须直接生成 `slot = godot_new_<Type>_with_<Type>(source_ptr)`
