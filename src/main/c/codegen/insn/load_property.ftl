...
<#switch genMode>
    ...
    <#--  If we are in the getter, then do not call the getter recursively  -->
        ...
        <#break>
    <#case "engine">
        $${insn.resultId} = godot_${objectType.typeName}_get_${insn.propertyName}($${insn.objectId});
        <#break>
    <#case "general">
        {
        ...
        <#else>
            ...
            $${insn.resultId} = ${helper.renderUnpackFunctionName(resultType)}(&__temp_variant);
        }
        <#break>
    <#case "builtin">
        <#if func.checkVariableRef(insn.objectId)>
            $${insn.resultId} = godot_${resultType.typeName}_get_${insn.propertyName}($${insn.objectId});
        <#else>
            $${insn.resultId} = godot_${resultType.typeName}_get_${insn.propertyName}(&$${insn.objectId});
        </#if>
        <#break>
